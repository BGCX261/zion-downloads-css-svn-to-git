...
foreach($admins AS $admin)
{
	...
	{
  		$admin['web_group'] = "Нет группы/Индивидуальные права";
	}
	if(empty($admin['server_group']) || $admin['server_group']==" ")
	{
		$admin['server_group'] = "Нет группы/Индивидуальные права";
	}
...
//=================[ Start Layout ]==================================
$admin_nav = 'Отображается&nbsp;'.$AdminsStart.'&nbsp;-&nbsp;'.$AdminsEnd.'&nbsp;из&nbsp;'.$admin_count.'&nbsp;результатов';
	...
	if(isset($_POST['new_override_name']))
	{
		...
		if(isset($_POST['override_id']))
		{
			...
			foreach($_POST['override_id'] as $index => $id)
			{
				...
				if(!empty($chk))
				{
					$edit_errors .= "&bull; Переопределение с таким именем уже существует \\\"" . htmlspecialchars(addslashes($_POST['override_name'][$index])) . "\\\" для этого типа.<br />";
					continue;
				}
			...
			if(!empty($edit_errors))
				throw new Exception("Произошла ошибка при применении изменений:<br /><br />" . $edit_errors);
		}
		...
		if(!empty($_POST['new_override_name']))
		{
			...
			if(!empty($chk))
				throw new Exception("Переопределение с таким именем уже существует для выбраного типа.");
