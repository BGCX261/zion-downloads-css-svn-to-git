...
function Plogin($username, $password, $remember, $redirect, $nopass)
{
	...
	{
		$objResponse->addScript('ShowBox("Информация", "Ваш аккаунт был импортирован из системы AMXBANS. Прежде, чем войти под вашим логином, ваш пароль должен быть пересоздан главным админом.", "blue", "", true);');
		return $objResponse;
	} else if(!$q || !$userbank->CheckLogin($userbank->encrypt_password($password), $aid))
	{
		if($nopass!=1)
			$objResponse->addScript('ShowBox("Некорректный логин", "Неверно введены имя пользователя или пароль.<br \> Если Вы забыли свой пароль, Используйте ссылку <a href=\"index.php?p=lostpassword\" title=\"Забыли пароль\">Забыли пароль</a> link.", "red", "", true);');
		return $objResponse;
	}
	else {
...
function CheckSrvPassword($aid, $srv_pass)
{
    ...
    if(!$userbank->is_logged_in() || $aid != $userbank->GetAid())
	{
		...
		$log = new CSystemLog("w", "Ошибка доступа", $username . " tried to check ".$userbank->GetProperty('user', $aid)."'s server password, but doesn't have access.");
		return $objResponse;
	}
...
function ChangeSrvPassword($aid, $srv_pass)
{
	...
	{
		$objResponse->redirect("index.php?p=login&m=no_access", 0);
		$log = new CSystemLog("w", "Ошибка доступа", $username . " tried to change ".$userbank->GetProperty('user', $aid)."'s server password, but doesn't have access.");
		return $objResponse;
	}
	...
	$objResponse->addScript("ShowBox('Пароль сервера сменён', 'Пароль сервера был успешно сменён.', 'green', 'index.php?p=account', true);");
	$log = new CSystemLog("m", "Сменён пароль сервера", "Пароль сменил админ (".$aid.")");
	return $objResponse;
}
...
function ChangeEmail($aid, $email, $password)
{
	...
	{
		$objResponse->redirect("index.php?p=login&m=no_access", 0);
		$log = new CSystemLog("w", "Ошибка доступа", $username . " пытался сменить e-mail админа ".$userbank->GetProperty('user', $aid).", не имея на это прав.");
		return $objResponse;
	}
	...
	$objResponse->addScript("ShowBox('E-mail сменён', 'Ваш e-mail успешно изменён.', 'green', 'index.php?p=account', true);");
	$log = new CSystemLog("m", "E-mail Сменён", "E-mail сменил админ (".$aid.")");
	return $objResponse;
}
...
function RemoveGroup($gid, $type)
{
	...
	{
		if(isset($rehashing))
			$objResponse->addScript("ShowRehashBox('".implode(",", $allservers)."', 'Группа удалена', 'Группа была успешно удалена', 'green', 'index.php?p=admin&c=groups', true);");
		else
			$objResponse->addScript("ShowBox('Группа удалена', 'Группа была успешно удалена', 'green', 'index.php?p=admin&c=groups', true);");
		$log = new CSystemLog("m", "Группа удалена", "Группа (" . $gid . ") удалена");
	}
	else
		$objResponse->addScript("ShowBox('Ошибка', 'Не получилось удалить группу. Смотрите системный лог для большей информации.', 'red', 'index.php?p=admin&c=groups', true);");
...
function RemoveSubmission($sid, $archiv)
{
	...
	if($archiv == "1") { // move submission to archiv
		...
		if($query1)
		{
			...
			$objResponse->addScript("ShowBox('Ошибка', 'Не получилось переместить заявку. Смотрите системный лог для подробной информации', 'red', 'index.php?p=admin&c=bans', true);");
		...
		if($query1)
		{
			$objResponse->addScript("ShowBox('Заявка удалена', 'Выбранная заявка была удалена успешно', 'green', 'index.php?p=admin&c=bans', true);");
			$log = new CSystemLog("m", "Заявка удалена", "Заявка (" . $sid . ") была удалена");
		}
		else
			$objResponse->addScript("ShowBox('Ошибка', 'Не получилось удалить заявку. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=bans', true);");
		...
		if($query1)
		{
			...
			$objResponse->addScript("ShowBox('Ошибка', 'Не получилось восстановить заявку. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=bans', true);");
	}
	return $objResponse;
}
...
function RemoveProtest($pid, $archiv)
{
	...
	if($archiv == '0') { // delete protest
		...
		if($query1)
		{
			$objResponse->addScript("ShowBox('Протест удалён', 'Выбранный протест был успешно удалён', 'green', 'index.php?p=admin&c=bans', true);");
			$log = new CSystemLog("m", "Протест удалён", "Протест (" . $pid . ") был удалён");
		}
		else
			$objResponse->addScript("ShowBox('Ошибка', 'Не получилось удалить протест. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=bans', true);");
		...
		if($query1)
		{
			$objResponse->addScript("ShowBox('Протест отправлен в архив', 'Выбранный протест был успешно отправлен в архив.', 'green', 'index.php?p=admin&c=bans', true);");
			$log = new CSystemLog("m", "Протест в архиве", "Протест (" . $pid . ") был отправлен в архив.");
		}
		else
			$objResponse->addScript("ShowBox('Ошибка', 'Не получилось отправить в архив протест. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=bans', true);");
		...
		if($query1)
		{
			$objResponse->addScript("ShowBox('Протест восстановлен', 'Протест был успешно восстановлен из архива.', 'green', 'index.php?p=admin&c=bans', true);");
			$log = new CSystemLog("m", "Протест восстановлен", "Протест (" . $pid . ") был восстановлен из архива.");
		}
		else
			$objResponse->addScript("ShowBox('Ошибка', 'Не получилось восстановить протест из архива. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=bans', true);");
	}
	return $objResponse;
}
...
function RemoveServer($sid)
{
	...
	if($query1)
	{
		$objResponse->addScript("ShowBox('Сервер удалён', 'Выбранный сервер был успешно удалён из базы', 'green', 'index.php?p=admin&c=servers', true);");
		$log = new CSystemLog("m", "Сервер удалён", "Сервер (" . $servinfo['ip'] . ":" . $servinfo['port'] . ") был удалён");
	}
	else
		$objResponse->addScript("ShowBox('Ошибка', 'Не получилось удалить сервер. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=servers', true);");
	return $objResponse;
}
...
function RemoveMod($mid)
{
	...
	if($query1)
	{
		$objResponse->addScript("ShowBox('МОД удалён', 'Выбранный МОД был успешно удалён', 'green', 'index.php?p=admin&c=mods', true);");
		$log = new CSystemLog("m", "МОД удалён", "МОД (" . $modicon['name'] . ") был удалён");
	}
	else
		$objResponse->addScript("ShowBox('Ошибка', 'Не получилось удалить МОД. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=mods', true);");
	return $objResponse;
}
...
function RemoveAdmin($aid)
{
	...
	{
		if(isset($rehashing))
			$objResponse->addScript("ShowRehashBox('".implode(",", $allservers)."', 'Админ удалён', 'Выбранный админ был удалён из базы', 'green', 'index.php?p=admin&c=admins', true);");
		else
			$objResponse->addScript("ShowBox('Админ удалён', 'Выбранный админ был удалён из базы', 'green', 'index.php?p=admin&c=admins', true);");
		$log = new CSystemLog("m", "Админ удалён", "Админ (" . $gid['user'] . ") был удалён");
	}
	else
		$objResponse->addScript("ShowBox('Ошибка', 'Не получилось удалить админа. Смотрите системный лог для получения подробной информации', 'red', 'index.php?p=admin&c=admins', true);");
	return $objResponse;
}
						...
						foreach($players AS $player) {
							...
							if($userbank->HasAccess(ADMIN_OWNER|ADMIN_ADD_BAN)) {
								$objResponse->addScript('AddContextMenu("#player_s'.$sid.'p'.$player["index"].'", "contextmenu", true, "Player Commands", [
														...
														{separator: true},
														'.(ini_get('safe_mode')==0 ? '{name: "Профиль", callback: function(){ViewCommunityProfile('.$sid.', "'.str_replace('"', '\"', $player["name"]).'")}},':'').'
...
function KickPlayer($sid, $name)
{
	...
	if($found) {
		...
		if($immune <= $userbank->GetProperty('srv_immunity')) {
			...
			$log = new CSystemLog("m", "Игрок кикнут", $username . " kicked player '".htmlspecialchars($name)."' (".$steam.") с сервера ".$data['ip'].":".$data['port'].".", true, true);
...
function PasteBan($sid, $name, $type=0)
{
	...
	if($found) {
		...
		$objResponse->addScript("ShowBox('Ошибка', 'Невозможно получить информацию о игроке ".addslashes(htmlspecialchars($name)).". Игрок покинул сервер (".$data['ip'].":".$data['port'].") anymore!', 'red', '', true);");
		$objResponse->addScript("$('dialog-control').setStyle('display', 'block');");
		return $objResponse;
	}
...
function AddMod($name, $folder, $icon, $steam_universe, $enabled)
{
	...
	if(!empty($check))
	{
		$objResponse->addScript("ShowBox('Ошибка при добавлении мода', 'Название мода уже используется.', 'red');");
		return $objResponse;
	}
...
function EditAdminPerms($aid, $web_flags, $srv_flags)
{
	...
	{
		$objResponse->addScript("ShowBox('Ошибка', Администраторы должны иметь пароль и адрес электронной почты для того, чтобы получить веб-разрешений..<br /><a href=\"index.php?p=admin&c=admins&o=editdetails&id=" . $aid . "\" title=\"Edit Admin Details\">Set the details</a> first and try again.', 'red', '');");
		return $objResponse;
	}
	...
	if(isset($GLOBALS['config']['config.enableadminrehashing']) && $GLOBALS['config']['config.enableadminrehashing'] == 1)
	{
		...
		$objResponse->addScript("ShowBox('Разрешения обновлены', 'Разрешения обновлены', 'Разрешения пользователя успешно обновлены', 'green', 'index.php?p=admin&c=admins');TabToReload();");
	$admname = $GLOBALS['db']->GetRow("SELECT user FROM `".DB_PREFIX."_admins` WHERE aid = ?", array((int)$aid));
...
function EditGroup($gid, $web_flags, $srv_flags, $type, $name, $overrides, $newOverride)
{
	...
	{
		$objResponse->redirect("index.php?p=login&m=no_access", 0);
		$log = new CSystemLog("w", "Ошибка доступа", $username . " пытался установить название группы. Это невозможно с нормальной формой.");
		return $objResponse;
	}
	...
	if($type == "srv")
	{
		...
		if(!empty($overrides))
		{
			foreach($overrides as $override)
			{
				...
				if(!empty($chk))
				{
					$objResponse->addScript("ShowBox('Ошибка', 'Там уже есть дублирование с именем \\\"" . htmlspecialchars(addslashes($override['name'])) . "\\\" выбранного типа..', 'red', '', true);");
					return $objResponse;
				}
...
function SendRcon($sid, $command, $output)
{
	...
	$objResponse->addScript("scroll.toBottom(); $('cmd').value=''; $('cmd').disabled=''; $('rcon_btn').disabled=''");
	$log = new CSystemLog("m", "РКОН отправлен", "РКОН был отправлен на сервер (".$rcon['ip'].":".$rcon['port']."): $command", true, true);
	return $objResponse;
}
...
function SendMail($subject, $message, $type, $id)
{
	...
	if($m)
	{
		$objResponse->addScript("ShowBox('E-mail отправлен', 'E-mail успешно отправлен.', 'green', 'index.php?p=admin&c=bans');");
		...
		$objResponse->addScript("ShowBox('Ошибка', 'Не удалось отправить e-mail.', 'red', '');");
...
function ApplyTheme($theme)
{
	...
	if($theme[0] == '.' || !in_array($theme, scandir(SB_THEMES)) || !is_dir(SB_THEMES . $theme) || !file_exists(SB_THEMES . $theme . "/theme.conf.php"))
	{
		$objResponse->addAlert('Неправильно выбран шаблон.');
		return $objResponse;
	}
	...
	if(!defined('theme_screenshot'))
	{
		$objResponse->addAlert('Ошибка выбранного шаблона.');
		return $objResponse;
	}
...
function RemoveComment($cid, $ctype, $page)
{
	...
	if($res)
	{
		$objResponse->addScript("ShowBox('Комментарий удалён', 'Комментарий был успешно удалён из базы', 'green', 'index.php$redir', true);");
		$log = new CSystemLog("m", "Comment Deleted", $username." deleted comment #".$cid);
	}
	else
		$objResponse->addScript("ShowBox('Ошибка', 'Ошибка удаления комментария из базы. Смотрите лог для подробной информации.', 'red', 'index.php$redir', true);");
	return $objResponse;
}
	...
	foreach ($links as $link) {
		if((strstr($link->getAttribute('href'), "http://steamcommunity.com/id/") || strstr($link->getAttribute('href'), "http://steamcommunity.com/profiles/")) && $link->childNodes->item(0)->nodeValue != "")
		{
			...
			$GLOBALS['db']->Execute($pre,array(0,
											   ...
											   0,
											   "Бан друга Steam Commynity (".htmlspecialchars($name).")",
											   $userbank->GetAid(),
											   $_SERVER['REMOTE_ADDR']));
		}
	}
...
function ViewCommunityProfile($sid, $name)
{
	...
	$data = $GLOBALS['db']->GetRow("SELECT ip, port, rcon FROM ".DB_PREFIX."_servers WHERE sid = '".$sid."';");
	if(empty($data['rcon'])) {
		$objResponse->addScript("ShowBox('Ощибка', 'Невозможно получить информацию о игроке ".addslashes(htmlspecialchars($name)).". Не задан РКОН пароль!', 'red', '', true);");
		return $objResponse;
	}
	$r = new CServerRcon($data['ip'], $data['port'], $data['rcon']);
